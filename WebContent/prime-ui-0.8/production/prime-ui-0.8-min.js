...
}if(this.options.forceSelection){this.cachedResults=[this.element.val()];
this.element.on("blur.puiautocomplete",function(){var d=a.element.val(),c=false;
for(var b=0;
b<a.cachedResults.length;
b++){if(a.cachedResults[b]==d){c=true;
break
}}if(!c){a.element.val("")
}})
...
}if(this.options.forceSelection){this.cachedResults=[];
this.data.each(function(h,j){g.cachedResults.push(j.label)
})
...
});$(function(){$.widget("primeui.puibutton",{options:{icon:null,iconPos:"left"},_create:function(){var b=this.element,d=b.text()||"pui-button",c=b.prop("disabled"),a=null;
